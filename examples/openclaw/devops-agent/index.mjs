...
export async function install() {
  ...
  console.log("Applying Terraform configuration for devops-agent...");
  await utils.terraform.apply(DIR);
  const ecrRepoUrl = await utils.terraform.output(DIR, { outputName: "ecr_repository_url" });

  // Build Docker image using shared bridge code + devops-agent Dockerfile
  console.log("Building devops-agent Docker image...");
  cd(SHARED_DIR);
  try {
