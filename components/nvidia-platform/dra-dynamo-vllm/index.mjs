...
export const name = "Dynamo vLLM Serving (DRA)";
...
export async function install() {
  ...
  // ============================================
  // DRA: Select DeviceClass
  // ============================================
  let deviceClassName = "gpu.nvidia.com";
  
  try {
    const dcResult = await $`kubectl get deviceclass -o jsonpath='{.items[*].metadata.name}'`.quiet();
    const classes = dcResult.stdout.trim().replace(/'/g, "").split(" ").filter(Boolean);
    const gpuClasses = classes.filter(c => ["gpu.nvidia.com", "mig.nvidia.com"].includes(c));
    
    if (gpuClasses.length === 0) {
      console.log("\n⚠️  No DRA DeviceClass found. Is the DRA Driver installed?");
      console.log("   Run: ./cli nvidia-platform dra install");
      process.exit(1);
    }
    
    if (gpuClasses.length === 1) {
      deviceClassName = gpuClasses[0];
      console.log(`\n✅ DRA DeviceClass: ${deviceClassName}`);
    } else {
      const { selectedClass } = await inquirer.prompt([{
        type: "list",
        name: "selectedClass",
        message: "Select DRA DeviceClass:",
        choices: gpuClasses.map(c => ({
          name: c === "gpu.nvidia.com" ? `${c} (full GPU)` : `${c} (MIG instance)`,
          value: c,
        })),
      }]);
      deviceClassName = selectedClass;
    }
  } catch {
    console.log("\n⚠️  Cannot query DeviceClasses. Using default: gpu.nvidia.com");
  }
