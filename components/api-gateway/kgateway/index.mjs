...
`;
  ...
  // Step 7: Provision cert-manager Pod Identity + CloudFront + WAF + Shield (Terraform)
  console.log("\nProvisioning Terraform (cert-manager IAM, CloudFront, WAF)...");
  const kgatewayConfig = config?.kgateway || {};
  let hostedZoneId = "";
  try {
    const hzResult = await $`aws route53 list-hosted-zones-by-name --dns-name ${process.env.DOMAIN} --query "HostedZones[0].Id" --output text`.quiet();
    hostedZoneId = hzResult.stdout.trim().replace("/hostedzone/", "");
  } catch { /* Route53 zone may not exist */ }
...
export async function uninstall() {
  ...
  await $`kubectl delete trafficpolicy increase-timeout no-auth-keycloak no-auth-oauth2-proxy -n ${NAMESPACE} --ignore-not-found`;
  ...
  // Destroy Terraform (cert-manager IAM, CloudFront, WAF, Shield)
  const kgatewayConfig = config?.kgateway || {};
  let hostedZoneId = "";
  try {
    const hzResult = await $`aws route53 list-hosted-zones-by-name --dns-name ${process.env.DOMAIN} --query "HostedZones[0].Id" --output text`.quiet();
